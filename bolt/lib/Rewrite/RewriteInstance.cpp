...
static cl::list<GadgetScannerKind> GadgetScannersToRun(
    ...
    cl::values(
        clEnumValN(GS_PAUTH_RETURN_TARGETS, "pauth-pac-ret",
                   "Unprotected returns (pac-ret)"),
        clEnumValN(GS_PAUTH_TAIL_CALLS, "pauth-tail-calls",
                   "Tail calls performed with unprotected link register"),
        clEnumValN(GS_PAUTH_BRANCH_AND_CALL_TARGETS, "pauth-forward-cf",
                   "Unprotected calls and branches (forward control-flow)"),
        clEnumValN(GS_PAUTH_SIGN_ORACLES, "pauth-sign-oracles",
                   "Signing of untrusted pointers (signing oracles)"),
        clEnumValN(GS_PAUTH_AUTH_ORACLES, "pauth-auth-oracles",
                   "Authentication oracles"),

        clEnumValN(GS_PAUTH_MASK, "pauth-all",
...
void RewriteInstance::runBinaryAnalyses() {
  ...
  uint64_t PAuthAnalysesMask = AnalysesMask & GSK::GS_PAUTH_MASK;
