...
enum GadgetScannerKind : uint64_t {
  ...
  GS_PAUTH_RETURN_TARGETS = (1 << 0),
  /// Scan for tail calls performed with untrusted link register.
  GS_PAUTH_TAIL_CALLS = (1 << 1),
  /// Scan for unprotected forward control-flow (branch and call instructions).
  GS_PAUTH_BRANCH_AND_CALL_TARGETS = (1 << 2),
  /// Scan for signing oracles.
  GS_PAUTH_SIGN_ORACLES = (1 << 3),
  /// Scan for authentication oracles.
  GS_PAUTH_AUTH_ORACLES = (1 << 4),

  /// Scan for any supported Pointer Authentication issues.
  GS_PAUTH_MASK = GS_PAUTH_RETURN_TARGETS | GS_PAUTH_TAIL_CALLS |
                  GS_PAUTH_BRANCH_AND_CALL_TARGETS | GS_PAUTH_SIGN_ORACLES |
                  GS_PAUTH_AUTH_ORACLES,

  /// Run all implemented scanners.
  GS_ALL_MASK = GS_PAUTH_MASK,
};
